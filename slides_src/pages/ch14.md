...
int main()
{
    char password[5] = "xyve";
    char input[5];
    printf("Enter your password: ");
    scanf("%s", input); // prevent overflow by scanf("%4s", input);
    if(strcmp(input, password)==0){
        // puts adds \n at the tail and not support format string
        puts("Access Granted!"); 
    } else {
        puts("Access Denied!");
    }
...
https://onlinegdb.com/Fyu6RzDGq

---


[Issues with this C code]{class="text-2xl"}

- Declaration Order & Stack Layout:

Declaring `char input[5]` after `char password[5] = "xyve"` typically places password at a higher memory address on architectures where the stack grows downward (when optimizations and stack reordering are disabled).

- Buffer Overflow & Write Direction:

The input array can hold only five bytes, including the null terminator.

Because `scanf("%s", input)` does not limit the number of characters read, entering five or more characters causes a buffer overflow.

The program may corrupt adjacent memory, crash, or appear to execute normally. C does not guarantee the outcome of undefined behavior.

> “Why doesn't C automatically throw an exception when we access memory outside an array?”
...
![ch14_2026-09-21-09-28-23](/images/ch14/ch14_2026-09-21-09-28-23.png){.max-h-50vh}
</div>
...
gc.disable()          # <-- stop the collector from reclaiming ANYTHING,
                       #     so every drop in free RAM is undeniably the leak

error_log = []
print("--- Starting Exception Leak (GC disabled) ---")

try:
    while True:
        try:
            raise ValueError("This is a heavy error message " + str(len(error_log)))
